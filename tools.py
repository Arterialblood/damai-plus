...
def account_login(login_type: str, login_id=None, login_password=None):
    ...
    if login_type == 'account':
        ...
        driver.find_element("name", 'fm-login-id').send_keys(login_id)
        driver.find_element("name", 'fm-login-password').send_keys(login_password)
        driver.find_element("class name", 'password-login').send_keys(Keys.ENTER)
